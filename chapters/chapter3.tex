...
The initial training configuration for the first draft of this thesis uses a batch size of 32, a learning rate of $10^{-3}$, the Adam optimizer, and cross-entropy loss for binary classification. Each experiment is trained for up to 20 epochs, and the best checkpoint is selected based on validation performance. An early stopping mechanism with a validation-based stopping criterion is used to reduce unnecessary computation and to limit overfitting. These values may be adjusted during pilot experimentation, but the same optimization procedure will be applied consistently across the core comparisons.

For transfer learning, the pretrained model is first adapted by replacing the original final classification head with a binary output layer. The initial fine-tuning plan is to begin with the pretrained backbone enabled and to optimize the network on the target dataset under the same experimental conditions used for the scratch-training baseline. If class imbalance is found to be substantial during dataset analysis, weighted loss or related balancing strategies may be tested as secondary experiments. However, the four main baseline comparisons remain fixed so that architecture and training strategy can be analyzed clearly.
